...
#include <stdlib.h> // For abs() and pow()
...
// color: 颜色
// num: 浮点数值
void LCD_ShowFloat(u16 x, u16 y, float num, u8 intLen, u8 decLen, u16 color)
{
    char buffer[20]; // 缓冲区，存储转换后的字符
    char *ptr = buffer;
    char *dotPos = buffer + intLen; // 小数点的位置
    int totalLen = intLen + decLen + 1; // 总长度，包括小数点

    // 将整数部分转换为字符串
    for (u8 i = 0; i < intLen; i++)
    {
        *ptr++ = (abs(num) / mypow(10, intLen - i - 1)) % 10 + '0';
        num -= (abs(num) / mypow(10, intLen - i - 1)) * my_pow(10, intLen - i - 1);
    }
    ...
    *ptr = '\0'; // 结束符
    ...
    {
        LCD_ShowChar(x + (8) * i, y, buffer[i], color);
    }
}
